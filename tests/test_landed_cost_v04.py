...
def test_landed_cost_includes_rates_fees_and_margin() -> None:
    ...
    assert result.landed_total == Decimal("148.00")
    assert result.landed_per_unit == Decimal("14.80")
    assert result.estimated_gross_profit == Decimal("52.00")
    assert result.estimated_gross_margin_pct == Decimal("26.00")
